  ...
  return (
    <div className="page-shell">
      <div className="page-header">
        ...
        <button className="btn-ghost btn-link" onClick={() => navigate('/my-apis')}>
          <ArrowLeft size={15} />
          <span>Back to My APIs</span>
        </button>
      </div>

      {error && <div className="inline-error block-error">{error}</div>}
      ...
      <div className="section-header">
        ...
        <button
          className="btn-ghost btn-link compact"
          onClick={() => setShowNewEndpoint(!showNewEndpoint)}
        >
          <Plus size={14} />
          <span>{showNewEndpoint ? 'Cancel' : 'Add endpoint'}</span>
        </button>
      </div>
        ...
        <div className="form-body form-section">
          <div className="grid2">
            ...
            <div className="field-row">
              ...
              <select value={newEndpoint.method} onChange={(e) => updateNewEp('method', e.target.value)}>
                <option>GET</option>
                <option>POST</option>
                <option>PUT</option>
                <option>PATCH</option>
          ...
          <button className="btn-gold btn-link" onClick={handleAddEndpoint} disabled={saving}>
            <Plus size={15} />
            <span>{saving ? 'Adding...' : 'Add endpoint'}</span>
          </button>
        </div>
      )}
